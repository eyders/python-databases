...
def import_investments(filename):
    ...
    with open(filename, 'r', encoding='utf-8') as csv_open:
        coin_reader = csv.reader(csv_open)
...
def view_investments(currency):
    ...
    coin_data = requests.get(url, timeout=50000).json()
    ...
    for investment in data:
        ...
        print(
            f"{investment.amount} {investment.coin} in {investment.currency} is worth {coin_total}")
